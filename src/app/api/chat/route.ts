...
export async function POST(req: NextRequest) {
  ...
  // 2) Retrieve via RPC (note: no generics, no any)
  const { data: matches, error } = await supabaseAnon.rpc('match_chunks', {
    query_embedding: vec,
    query_text: userMsg,
    match_count: 10,
  ...
  const rows: DbMatch[] = (matches ?? []) as unknown as DbMatch[];

  // Join titles for citations
  const withTitles: MatchWithTitle[] = await Promise.all(
    rows.map(async (m): Promise<MatchWithTitle> => {
      const { data: docTitle } = await supabaseAnon
        .from('documents')
        .select('title')
        .eq('id', m.doc_id)
        .single();
      const title: string | null = (docTitle?.title ?? null) as string | null;
      return { ...m, title };
    }),
  );
